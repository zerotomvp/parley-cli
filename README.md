...
Parley is a free tool in the [Zero to MVP toolkit](https://zerotomvp.dev/toolkit).
[Explore the toolkit and subscribe for updates](https://zerotomvp.dev/toolkit), or
[tell us what tool is missing](https://zerotomvp.dev/toolkit#feedback).
...
- [Self-guiding, context-efficient output](#self-guiding-context-efficient-output)
- [Roles and session identity](#roles-and-session-identity)
- [Managing members](#managing-members)
- [Scope and limitations](#scope-and-limitations)
- [Acknowledging longer work](#acknowledging-longer-work)
- [Enable automatic wake-up](#enable-automatic-wake-up)
...
Official releases are published through GitHub Releases, NuGet, Homebrew, and
Scoop. Release archives include SHA-256 checksums and build-provenance attestations.
Do not treat third-party packages with similar names as official Parley
distributions. See [`CHANGELOG.md`](CHANGELOG.md) for release history.
...
prerelease cannot resolve to a stable package from another NuGet feed. It warns
before installing a dirty worktree. To build without changing your global tools,
run `dotnet build -c Release` instead. For an isolated install, use
`./install.sh --tool-path /path/to/tools`; `--force` permits a non-interactive
...
Every receive declares a model-context checkpoint: the highest Parley sequence
actually present in the caller's context. Use `0` before seeing any message:
...
This model-context checkpoint can replay messages when a previous CLI process emitted
...
Parley is self-guiding after an agent has joined, but it cannot discover the channel
name or the role assigned out of band. Give a new participant this one bootstrap
prompt:
...
The prompt is deliberately explicit about facts the CLI cannot infer. From that
point, `join`, wake notices, receive results, checkpoint footers, and command help
provide the next action and recovery command.
...
After the bootstrap prompt, a coding agent should not need a person to keep
translating Parley's protocol. Command-level `--help`, `join` instructions,
...
another session is rejected. After a same-harness runtime restart, reclaim the role
with `join --force`; a new session's receive position starts at the current end of
the transcript so it resumes forward rather than draining the historical backlog.
...
- Delivery is durable, but automatic wake-up is best-effort. Receivers recover
  using their `--last-seen` model-context checkpoint.
...
Automatic wake-up is an optional notification layer over the durable transcript.
Install the integration for the harness you use; message delivery and checkpoint
recovery continue to work if a wake attempt fails. Protocol details and invariants
belong in [`SPEC.md`](SPEC.md).
...
`/clear`, `/compact`, and resume behavior is covered by the
[Claude lifecycle smoke test](docs/claude-lifecycle-smoke.md); the underlying
session-rebinding protocol is documented in [`SPEC.md`](SPEC.md).
...
The package starts Parley's local wake helper with each Pi session. The wake notice
does not advance a checkpoint; the model still runs the displayed nonblocking
`recv --last-seen` command to consume the durable message.
...
If the extension is unavailable or stale, `join` reports it and prints the
foreground-listener fallback. See [`SPEC.md`](SPEC.md) for the Pi bridge protocol.
...
Each send uses only the transport stored for the destination role. There is no
send-side wake option, and wake failure never undelivers the transcript message.
Retry and duplicate-notice behavior is specified in [`SPEC.md`](SPEC.md).
...
- With automatic Claude, Pi, or Codex wake available, do not maintain a blocking
  listener. An incoming send injects an event or starts/steers the loaded thread.
  Use a plain `recv --last-seen <seq>` for catch-up or recovery.
...
timeout, which is why `--last-seen` remains authoritative.
...
The CLI targets .NET 10 and is designed for Linux, macOS, and Windows. Release
archives target `linux-x64`, `linux-arm64`, `osx-x64`, `osx-arm64`,
...
Most users need no environment variables. These are the supported operator-facing
overrides:
...
Harness-provided variables such as `CODEX_THREAD_ID`, `CLAUDE_CODE_SESSION_ID`,
`PI_CODING_AGENT`, and `PI_SESSION_ID` are detected automatically; users should not
normally set them. Test-only variables are documented under [Development](#development).
